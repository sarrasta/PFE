# RAPPORT ACADÉMIQUE — VOLET MACHINE LEARNING
...
## Guide de lecture scientifique et cadre de sélection

Ce rapport constitue la version académique centrée sur le volet Machine Learning du PFE. Pour chacun des neuf objectifs, il répond à quatre questions de soutenance : **pourquoi cet objectif est nécessaire**, **quelles approches ont été évaluées**, **selon quel protocole elles ont été comparées**, et **pourquoi la méthode finale a été retenue**. Les valeurs proviennent exclusivement des notebooks et journaux d'exécution versionnés dans `ml pfe` ; elles ne sont ni extrapolées ni réestimées a posteriori.

### Synthèse des décisions par objectif

| Objectif | Besoin décisionnel | Approches évaluées | Critère déterminant | Choix final et justification |
|---|---|---|---|---|
| 1 — Churn | Classer le risque de départ | LR, arbres, RF, GB, XGBoost, LightGBM, SVC, NuSVC, ensembles ; SMOTE, SVMSMOTE, CTGAN, TVAE, KDE et copules | AUC-ROC en CV stratifiée, sans fuite | **SVC-RBF + SMOTE** : meilleure AUC fiable, 0,7138 ± 0,1144 |
| 2 — Risque | Convertir plusieurs fragilités en niveaux opérationnels | K-Means pour plusieurs valeurs de k, puis RF de déploiement | Cohérence métier, silhouette et fidélité au clustering | **K-Means k=3 + RF** : trois niveaux interprétables ; RF à 97,34 % |
| 3 — Métriques métier | Transformer un score en règle de contact rentable | SVC, LR-SMOTE, RF équilibré et GB équilibré | AUC, PR-AUC, Lift@K, Brier et valeur nette | **Choix multi-critère** : SVC pour le ranking, GB pour la calibration, seuil économique pour l'action |
| 4 — Segmentation | Personnaliser les actions selon le comportement | K-Means brut 23D, hiérarchique, PCA, transformations et scores composites v2/v3 | Silhouette, Davies-Bouldin et interprétabilité | **K-Means métier 4D v3** : silhouette 0,2324 et segments actionnables |
| 5 — Profils | Relier diagnostic et campagne | Matrice segment × risque et priorisation financière | Couverture, explicabilité et ROI | **12 micro-profils** : une règle opérationnelle par combinaison |
| 6 — Propension | Produire une priorité continue | GBR, RF Regressor, Ridge et SVR ; pondération et calibration isotonique | AUC de ranking et dispersion des scores | **Ridge calibré** : AUC 0,6772, supérieure aux modèles non linéaires |
| 7 — ARPU | Estimer la valeur future | Ridge, Gradient Boosting et Random Forest | Split temporel, R², MAE, RMSE et MAPE | **Random Forest** : R² 0,9619 et MAE 1,77 TND |
| 8 — Réponse | Estimer la réceptivité à l'offre | Régression logistique, GB et RF calibrés | AUC, Average Precision et Brier | **LogReg calibrée** : meilleure discrimination, AUC 0,6185 |
| 9 — Gain | Prioriser sous contrainte budgétaire | Espérance analytique, scénarios coût/LTV et sensibilité | Gain net et Lift@TopK | **Espérance économique** : lift 2,68× sur le Top 10 % |

### Principe de comparaison

Une valeur élevée ne suffit pas à justifier un modèle. La sélection considère simultanément la généralisation hors échantillon, l'absence de fuite de cible, la stabilité entre plis, la calibration, l'interprétabilité et le coût opérationnel. Cela explique notamment le rejet du Random Forest de l'Objectif 1 malgré un F1 proche de 0,96 : avec 93,6 % de churners, ce score peut être obtenu en favorisant presque systématiquement la classe majoritaire, alors que l'AUC reste inférieure à 0,5.
